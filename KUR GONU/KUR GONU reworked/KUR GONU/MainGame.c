...
{			
	...
	if( mode == _MODE_F ){
		...
		logWriter(" ");
		piecesDeploy( tempSetSpotsP1 );// piecesDeploy( tempSpotsP[1] )이렇게 매개변수가 setSpot[][MAX_PIECE_NUM]이 되어야 한다.
	...
	initPrintPieces( piecesP1, piecesP2 );// initPrintPieces( piecesP[1], piecesP[2] ); //매개변수는 pieces[][PIECE_NUM_MAX] 로 선언
...
{
	int focusP1 = -1;	// 방향키에 의한 이동을 위한 변수 focus
	int focusP2 = -1;
	int input;
